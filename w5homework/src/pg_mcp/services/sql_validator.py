...
class SQLValidator:
    ...
    def __init__(
        ...
        Args:
            ...
            allowed_tables: Optional list of allowed table names. If set, only these are allowed.
            allow_explain: Whether to allow EXPLAIN statements. If None, uses config value.
        """
        self.config = config
        
        # Merge config blocked tables with runtime args
        config_blocked = set(config.blocked_tables) if hasattr(config, "blocked_tables") else set()
        arg_blocked = set(blocked_tables or [])
        self.blocked_tables = {t.lower() for t in (config_blocked | arg_blocked)}
        
        # Setup allowed tables (config + runtime)
        config_allowed = set(config.allowed_tables) if hasattr(config, "allowed_tables") else set()
        arg_allowed = set(allowed_tables or [])
        self.allowed_tables = {t.lower() for t in (config_allowed | arg_allowed)}

        self.blocked_columns = {c.lower() for c in (blocked_columns or [])}
        
        # Use arg if provided, otherwise config, otherwise False
        if allow_explain is not None:
             self.allow_explain = allow_explain
        elif hasattr(config, "allow_explain"):
             self.allow_explain = config.allow_explain
        else:
             self.allow_explain = False
    ...
    def _check_allowed_tables(self, statement: exp.Expression) -> str | None:
        ...
        Returns:
            Error message if check fails, None otherwise.
        """
        # If no allowed list is defined, all tables are allowed (unless blocked)
        ...
        for table in statement.find_all(exp.Table):
            ...
            # Skip empty table names (can happen with some CTEs or complex queries)
            if not table_name:
                continue

            if table_name not in self.allowed_tables:
                return f"Access to table '{table_name}' is not in the allowed list"
